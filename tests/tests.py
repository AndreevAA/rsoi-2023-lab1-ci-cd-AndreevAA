...
def test_get_all_persons():
    r = requests.get(url="http://localhost:3000/api/v1/persons")
    assert r.status_code == 200
    assert len(r.json()) > 0


def test_get_person_not_found():
    r = requests.get(url="http://localhost:3000/api/v1/persons/100")
    assert r.status_code == 400
    assert r.text == "Not found Person for ID 100"


def test_post_invalid_data():
    r = requests.post(url="http://localhost:3000/api/v1/persons", json={})
    assert r.status_code == 400
    assert r.text == "Invalid data"


def test_patch_person_not_found():
    r = requests.patch(url="http://localhost:3000/api/v1/persons/100", json=patch_name)
    assert r.status_code == 404
    assert r.text == "Not found Person for ID 100"


def test_delete_person_not_found():
    r = requests.delete(url="http://localhost:3000/api/v1/persons/100")
    assert r.status_code == 404
...
def test_post_get_many():
    for i in range(100):
        r = requests.post(url="http://localhost:3000/api/v1/persons", json=person1)
        assert r.status_code == 201
        redirected_urd = r.headers['Location']
        r = requests.get(redirected_urd)
        assert r.status_code == 200
